...
router.get("/changepage/:init", function(req, res, next) {
  Database.query(
    ...
    function(err, data) {
      console.log(
        `select * from products as p , productlines as pl where p.productLine = pl.productLine where quantityInStock <> 0 limit ${req.params.init},15`
      );
      res.json(data);
    }
  );
...
router.post("/addCart", function(req, res, next) {
  ...
  Database.query(
    `SELECT * FROM products join productlines using (productLine) WHERE productCode = '${code}'`,
    (err, data) => {
      var check = false;
      let i = 0;
      let targetIndex;
      ...
      if (!check) {
        ...
        req.session.cartItem[targetIndex].Total +=
          quantity * req.session.cartItem[targetIndex].Price;
      }
...
router.delete("/removeCartItem/:code", function(req, res, next) {
  ...
  var currentAmount =
    parseInt(req.session.totalQuantitiy) -
    parseInt(req.session.cartItem[targetIndex].Quantity);
